...
# Keep a global reference to the session and user context
session_ref = {"session": None, "user_context": None}
...
async def entrypoint(ctx: agents.JobContext):
    ...
    session_ref["user_context"] = user_context
...
async def end_agent_session():
    ...
    if session:
        await session.end()
    # Reset user context after session ends
    session_ref["user_context"] = None
