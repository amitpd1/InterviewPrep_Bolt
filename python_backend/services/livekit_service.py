...
class LiveKitService:
    ...
    #     logger.info("Checking LiveKitService config")
    #     return bool(self.api_key and self.api_secret and self.ws_url)

    def is_configured(self):
        logger.info("🔍 LiveKitService config check (full values):")
        logger.info(f"  - API Key: {self.api_key}")
        logger.info(f"  - API Secret: {self.api_secret}")
        logger.info(f"  - WS URL: {self.ws_url}")

        result = bool(self.api_key and self.api_secret and self.ws_url)
        logger.info(f"  => is_configured() = {result}")
        return result
